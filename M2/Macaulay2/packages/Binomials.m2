...
document {
    ...
    EXAMPLE {
        ...
        "isBinomial J"},
    "Next example shows that, compared to computing a Gröbner basis, detecting
    binomiality with Gröbner free methods is very fast (see Chapter 2 §10 in [CLO15]):",
    EXAMPLE {
        "R=QQ[x,y,z,w]",
        "n=40",
...
R = QQ[x]
I = ideal (x,x^2)
assert( gensMinimalDegree I == matrix{{x}})

K = frac(QQ[a,b])
R = K[x,y]
I = ideal (b^2*x-y, a*x-b*y, a*b*x^3*y)
assert (gensMinimalDegree I == matrix{{a*x-b*y, b^2*x-y}})
///
