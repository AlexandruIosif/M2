...
--the follwong returns the row echelon form of a matrix with the row order inverted
inverseRowEchelon = C ->(
    c:=numColumns C;
    variable := symbol variable;
    R:=ring (C_(0,0)); S:=R[variable_1..variable_c];
    C=sub(transpose(coefficients(gens gb ideal flatten entries (sub(C,S)*transpose(vars S)),Monomials=> flatten entries vars S))_1,R);
C)

-- TEST ///
-- assert (inverseRowEchelon matrix{{1,0},{0,1}} == (matrix{{0, 1}, {1, 0}}))
-- ///


--Not implemented yet:
--When it will be implemented it will compute quotient rings by
--homogeneous ideals without Groebner basis Maybe it is good to
--implement a time case function: if R/I takes a lot of time, use
--another method, etc.  Eg. if R/I takes a lot of time but I is
--generated in degree small enough, try R/(I+I_{d+1}), where I_{d+1}
--is the ideal generated by all the monomials of degree d+1. Maybe we
--can compute in paralle in many ways and get the fastest result?
quotientRing = method ()
quotientRing (Ring,Ideal) := QuotientRing => (R,I) -> (
    S:=R/I; --needs improvement; it is not Groebner free yet!!!
    --ISN'T IT??? IT IS QUITE FAST EVEN FOR an ideal I, WORST CASE;
    --see example bellow
    S)
...
--detects whether or not a matrix has at most two non zero entries en each row
...
isBinomialGroebnerFree = I ->(    
    ...
    if (isHomogeneous ideal(Fl)) == false then( print "Error: Non-homogeneuos ideals"; return;);

    while (F=!=set{} and F=!=set{0_R}) do(
	Fmin=flatten entries gensMinimalDegree(ideal toList F);
        (M,A)=coefficients (matrix{Fmin}, Monomials => toList set flatten entries monomials matrix{toList F});
	A= inverseRowEchelon transpose (A);
	M=transpose M;
	if isSupportOfRowsAtMostTwo(A)===false then return false;
	B=B|flatten entries sub(sub(A*M,R),S);
	d=(degree (A*M)_0_0)_0;
	F=F-(set Fmin);
--	Id=sub((ideal gens S)^(d+1),R); -- this makes the algorithm
--	extremely slow; somehow, the quotient R/I works very fast; Id
--	is huge and it takes very longo
	if F === set{} then return (true,matrix{B}); --this avoids the next step in case F is empty
	R=quotientRing(R,ideal flatten entries (A*M));
	F = set flatten entries gens substitute(ideal toList F+Id,R);
	F=F-set{0_R};
	);
    )
